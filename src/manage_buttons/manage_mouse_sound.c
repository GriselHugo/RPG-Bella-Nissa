...
{
    sfFloatRect hitbox_s_m = {game->option.soundminus.pos.x,
    game->option.soundminus.pos.y, 80, 80};
    if (sfFloatRect_contains(&hitbox_s_m, mouse_pos.x, mouse_pos.y)) {
        game->option.soundminus.rect.top = 80;
        if (game->event.type == sfEvtMouseButtonPressed) {
            game->option.soundminus.rect.top = 80 * 2;
            game->option.soundminus.status = 1;
        }
        if (game->event.type == sfEvtMouseButtonReleased &&
        game->option.soundminus.status == 1) {
            change_volume(game, 's', -10);
            game->option.soundminus.status = 0;
        }
    } else {
        game->option.soundminus.rect.top = 0;
        game->option.soundminus.status = 0;
    }
    sfSprite_setTextureRect(game->option.soundminus.sprite,
    game->option.soundminus.rect);
}

void manage_sound_plus_button(game_t *game, sfVector2i mouse_pos)
{
    sfFloatRect hitbox_s_p = {game->option.soundplus.pos.x,
    game->option.soundplus.pos.y, 80, 80};
    if (sfFloatRect_contains(&hitbox_s_p, mouse_pos.x, mouse_pos.y)) {
        game->option.soundplus.rect.top = 80;
        if (game->event.type == sfEvtMouseButtonPressed) {
            game->option.soundplus.rect.top = 80 * 2;
            game->option.soundplus.status = 1;
        }
        if (game->event.type == sfEvtMouseButtonReleased &&
        game->option.soundplus.status == 1) {
            change_volume(game, 's', 10);
            game->option.soundplus.status = 0;
        }
    } else {
        game->option.soundplus.rect.top = 0;
        game->option.soundplus.status = 0;
    }
    sfSprite_setTextureRect(game->option.soundplus.sprite,
    game->option.soundplus.rect);
}
